...
(function() {
  ...
  async function refreshNotifCount() {
    try {
      const r = await fetch('/api/notifications/counters');
      if (!r.ok) throw new Error('HTTP ' + r.status);
      const d = await r.json();
      const reqCount = Number(d.requests != null ? d.requests : (d.pending || 0));
      const chatUnread = Number(d.chat_unread || 0);
      updateNotifBadge(reqCount);
      updateChatBadge(chatUnread);
    } catch (_) {
      // Фолбэк на старое поведение, если новый эндпоинт недоступен
      try {
        let count = 0;
        const r2 = await fetch('/api/requests/count');
        if (r2.ok) {
          const d2 = await r2.json();
          count = Number(d2.count || 0);
        } else {
        ...
        // тихо глотаем, чтобы не спамить консоль
      }
    }
  }
  ...
  async function rejectRequest(id) {
    try {
      ...
      if (window.notify && typeof window.notify.success === 'function') {
        window.notify.success('Запрос отклонён');
      } else {
      ...
      if (window.notify && typeof window.notify.error === 'function') {
        window.notify.error('Ошибка отклонения запроса');
      } else {
  ...
  async function approveRequest(id) {
    try {
      ...
      if (window.notify && typeof window.notify.error === 'function') {
        window.notify.error('Не удалось открыть запрос');
      } else {
        showToast('Не удалось открыть запрос', 'error');
      }
    }
  }
  ...
  // Экспорт для других модулей (например, WebSocket chat.js может попросить
  // обновить счётчики, чтобы колокольчик/чат не ждали следующего polling).
  window.refreshNotifCount = refreshNotifCount;
})();
