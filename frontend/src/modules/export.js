...
(function() {
  ...
  function exportGeoJSON() {
    ...
    } catch (e) { console.error(e); showToast('Ошибка экспорта', 'error'); }
  }
  function openImportJson() { const hf = document.getElementById('hidden-json'); if (hf) hf.click(); }
  async function handleImportJson(e) {
    const file = e.target.files[0]; if (!file) return;
    try {
      const text = await file.text();
      const data = JSON.parse(text);
      if (!data || !Array.isArray(data.features)) { showToast('Некорректный файл GeoJSON', 'error'); e.target.value=''; return; }
      for (const feat of data.features) {
        ...
        const payload = {
          address: props.address || '', lat: lat || null, lon: lon || null,
          description: props.description || '', status: props.status || '',
          link: props.link || '', category: props.category || ''
        };
      ...
      e.target.value = ''; await refresh();
    } catch (err) { console.error(err); showToast('Ошибка импорта', 'error'); e.target.value=''; }
